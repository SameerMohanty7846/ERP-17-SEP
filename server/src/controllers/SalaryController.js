...
export const createSalaryComponent = async (req, res) => {
    const { name, type, is_days_based } = req.body;
    try {
        const newComponent = await SalaryComponent.create({ name, type, is_days_based });
...
export const updateSalaryComponent = async (req, res) => {
    ...
    try {
        ...
        await component.update({ name, type, is_days_based });
