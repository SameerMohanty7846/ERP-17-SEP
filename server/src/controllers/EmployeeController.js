...
//with employee password history
export const createEmployee = async (req, res) => {
  try {
    const { name, email, phone, address, joined_at } = req.body;
    ...
    if (name.length >= 2 && phone?.length >= 5) {
      const namePart = name.charAt(0).toUpperCase() + name.charAt(1).toLowerCase();
      const phonePart = phone.slice(-5);
      rawPassword = `${namePart}@${phonePart}`;
    }
    ...
    if (!newEmployee || !newEmployee.id || !hashedPassword) {
      throw new Error("Failed to create employee or generate password.");
    }

    // Save password to EmployeePassword history table
